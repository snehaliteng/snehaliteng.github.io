...
serve(async (req) => {
  ...
  try {
    ...
    const { data: plan, error: planError } = await supabase.from('qna_plans').select('*').eq('id', plan_id).single()
    ...
    const rzpRes = await fetch('https://api.razorpay.com/v1/orders', {
      ...
      body: JSON.stringify({ amount: plan.price, currency: 'INR', receipt: `plan_${plan_id}_${user.id.slice(0, 8)}_${Date.now()}` })
    })
