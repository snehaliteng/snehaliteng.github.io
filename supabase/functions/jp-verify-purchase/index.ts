...
serve(async (req) => {
  ...
  try {
    ...
    const { data: plan } = await supabase.from('jp_plans').select('*').eq('id', plan_id).single()
    ...
    const now = new Date()
    const endsAt = plan.duration_days > 0
      ? new Date(now.getTime() + plan.duration_days * 24 * 60 * 60 * 1000).toISOString()
      : null

    const { error: upsertError } = await supabase.from('jp_subscriptions').insert({
      user_id: user.id,
      plan_id: plan_id,
      status: 'active',
    ...
    if (plan.target === 'company') {
      const planKey = plan.name.toLowerCase().includes('enterprise') ? 'enterprise'
        : plan.name.toLowerCase().includes('premium') ? 'premium' : 'free'
      await supabase.from('jp_companies').update({ plan: planKey }).eq('user_id', user.id)
    }
