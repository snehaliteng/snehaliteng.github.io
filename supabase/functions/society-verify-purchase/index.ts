...
serve(async (req) => {
  ...
  try {
    ...
    const { data: plan } = await supabase.from('society_plans').select('*').eq('id', plan_id).single()
    ...
    const endDate = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString().replace('T', ' ').substring(0, 19)

    const { error: upsertError } = await supabase.from('society_purchases').upsert({
      user_id: user.id,
      plan_id: plan_id,
