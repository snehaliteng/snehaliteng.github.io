...
serve(async (req) => {
  ...
  try {
    ...
    const { data: plan, error: planError } = await supabase.from('jp_plans').select('*').eq('id', plan_id).single()
    ...
    if (!plan.is_active) return new Response(JSON.stringify({ error: 'Plan not active' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    ...
    const rzpRes = await fetch('https://api.razorpay.com/v1/orders', {
      ...
      body: JSON.stringify({ amount: plan.price, currency: 'INR', receipt: `jp_plan_${plan_id}_${user.id.slice(0, 8)}_${Date.now()}` })
    })
