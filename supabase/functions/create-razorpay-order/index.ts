...
serve(async (req) => {
  try {
    ...
    if (!keyId || !keySecret) return new Response(JSON.stringify({ error: 'Razorpay not configured' }), { status: 500 })
    ...
    if (!rzpRes.ok) {
      ...
      return new Response(JSON.stringify({ error: 'Razorpay order failed', details: errBody }), { status: 502 })
    }
