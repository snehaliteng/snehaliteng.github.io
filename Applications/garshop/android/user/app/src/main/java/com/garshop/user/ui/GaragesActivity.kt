...
class GaragesActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Thread {
            try {
                val garages = Supabase.select("gs_garages", "status=eq.approved&select=id,name,location,city,phone,services_offered&order=created_at.asc")
                runOnUiThread {
                    list.removeAllViews()
                    if (garages.length() == 0) {
                        list.addView(TextView(this).apply { text = "No approved garages yet." })
