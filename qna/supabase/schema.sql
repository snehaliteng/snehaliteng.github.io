...
DROP POLICY IF EXISTS "admin manage plans" ON qna_plans;
CREATE POLICY "admin manage plans" ON qna_plans FOR ALL USING (auth.jwt() ->> 'email' = 'snehaliteng@gmail.com');
DROP POLICY IF EXISTS "users read active plans" ON qna_plans;
...
DROP POLICY IF EXISTS "users own plan" ON qna_user_plans;
CREATE POLICY "users own plan" ON qna_user_plans FOR SELECT USING (user_id = auth.uid());
DROP POLICY IF EXISTS "users insert own plan" ON qna_user_plans;
CREATE POLICY "users insert own plan" ON qna_user_plans FOR INSERT WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS "users update own plan" ON qna_user_plans;
CREATE POLICY "users update own plan" ON qna_user_plans FOR UPDATE USING (user_id = auth.uid());

GRANT SELECT ON qna_plans TO anon, authenticated;
GRANT ALL ON qna_plans TO authenticated;
GRANT SELECT, INSERT, UPDATE ON qna_user_plans TO authenticated;

-- Auto-increment sequences (run once)
CREATE SEQUENCE IF NOT EXISTS qna_categories_id_seq START WITH 1;
CREATE SEQUENCE IF NOT EXISTS qna_questions_id_seq START WITH 1;
CREATE SEQUENCE IF NOT EXISTS qna_answers_id_seq START WITH 1;
CREATE SEQUENCE IF NOT EXISTS qna_tags_id_seq START WITH 1;
CREATE SEQUENCE IF NOT EXISTS qna_job_applications_id_seq START WITH 1;
SELECT setval('qna_categories_id_seq', COALESCE((SELECT MAX(id) FROM qna_categories), 0) + 1);
SELECT setval('qna_questions_id_seq', COALESCE((SELECT MAX(id) FROM qna_questions), 0) + 1);
SELECT setval('qna_answers_id_seq', COALESCE((SELECT MAX(id) FROM qna_answers), 0) + 1);
SELECT setval('qna_tags_id_seq', COALESCE((SELECT MAX(id) FROM qna_tags), 0) + 1);
SELECT setval('qna_job_applications_id_seq', COALESCE((SELECT MAX(id) FROM qna_job_applications), 0) + 1);
ALTER TABLE qna_categories ALTER COLUMN id SET DEFAULT nextval('qna_categories_id_seq');
ALTER TABLE qna_questions ALTER COLUMN id SET DEFAULT nextval('qna_questions_id_seq');
ALTER TABLE qna_answers ALTER COLUMN id SET DEFAULT nextval('qna_answers_id_seq');
ALTER TABLE qna_tags ALTER COLUMN id SET DEFAULT nextval('qna_tags_id_seq');
ALTER TABLE qna_job_applications ALTER COLUMN id SET DEFAULT nextval('qna_job_applications_id_seq');
